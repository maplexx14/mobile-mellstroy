...
const isPortrait = (): boolean => resolution.cssClass === "ui-portrait";

const getLayoutConfig = (count: number): LayoutConfig => {
    if (isPortrait() && count > 9) {
        // Portrait: a 4-column grid of small tiles, like the mobile level select.
        return {
            columns: 4,
            leftOffset: -322,
            topOffset: 0,
...
const positionLevelButton = (
    ...
    // Clamp to 0 so grids taller than 5 rows (portrait, 4 columns) stay
    // top-anchored instead of being pulled above the top edge.
    const verticalOffset = Math.max(0, (fullHeight - totalHeight) / 2);

    levelElement.style.left = `${leftOffset + column * inc + rowOffset}px`;
    levelElement.style.top = `${topOffset + row * inc + verticalOffset}px`;

    levelElement.classList.toggle("option-small", layout.small ?? layout.columns === 5);
};
