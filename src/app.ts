...
class App {
    ...
    domReady(): void {
        ...
        // The ui-<width> class is keyed to the asset set (a portrait profile reuses
        // the 1920 assets), so element/sprite rules apply; cssClass adjusts geometry.
        document.body.classList.add(`ui-${resolution.UI_ASSET_WIDTH ?? resolution.UI_WIDTH}`);
        if (resolution.cssClass) {
            document.body.classList.add(resolution.cssClass);
        }
        if (IS_XMAS) {
